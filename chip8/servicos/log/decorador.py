...
def _log_str_formato(func_name, associacao_argumento_parametro_str: str, resultado_str: str):
    return f"Função: {func_name}, Parametros: {associacao_argumento_parametro_str}, Resultado: {resultado_str}"


def _formatar_str_log_resultado(a_ser_formatado, formatador: Callable[[Any], str]) -> str:
    str_inicial = "["
    str_final = "]"
...
def _formatar_str_log_parametros(a_ser_formatado: Dict, formatador: Callable[[Any], str]) -> str:
    str_inicial = "{"
    str_final = "}"
...
def _associar_argumentos_com_parametros(func: Callable, args: Sequence, kwargs: Dict) -> Dict:
    ...
    quantidade_parametros_faltantes = len(nome_params_func) - \
        len(parametros_args_e_kwargs_fornecidos)
    ...
    if quantidade_parametros_faltantes == qtd_parametros_padrao:
        return dict(zip(nome_params_func, _juntar_parametros(args, kwargs, parametros_padrao)))

    elif quantidade_parametros_faltantes < qtd_parametros_padrao:
        differenca_parametros_fornecidos_e_padrao = qtd_parametros_padrao - \
            quantidade_parametros_faltantes

        parametros_padrao_para_completar_parametros_faltantes = parametros_padrao[differenca_parametros_fornecidos_e_padrao: len(
            parametros_padrao) + 1]

        return dict(zip(nome_params_func, _juntar_parametros(args, kwargs, parametros_padrao_para_completar_parametros_faltantes)))

    elif quantidade_parametros_faltantes == 0:
        return dict(zip(nome_params_func, _juntar_parametros(parametros_args_e_kwargs_fornecidos)))

    else:
        raise Exception()


def _juntar_parametros(args: Sequence, kwargs: Dict = {}, padrao: Iterable[inspect.Parameter] = []) -> Sequence:
    return list(args) + list(kwargs.values()) + [p.default for p in padrao if p.default != inspect.Parameter.empty]
