...
from chip8.nucleo.dados.tipos import CONTADOR, CONTEXTO_RUNTIME, CONTEXTO_RUNTIME_FUNC_ATUALIZAR, CONTEXTO_RUNTIME_KEYS, INSTRUCAO_COMPLETA_CHIP8, PIXEL_MAP, RAM, REGISTRADOR_INDEX, REGISTRADORES, e_contador, e_instrucao, e_pixel_map, e_registrador, e_registrador_index
...
def escrever_varios_valores_contexto_runtime(contexto_runtime: CONTEXTO_RUNTIME, relacao_chave_valor: Mapping[CONTEXTO_RUNTIME_KEYS, Any]) -> CONTEXTO_RUNTIME:
    _validar_varios_inputs(relacao_chave_valor)
    with contexto_runtime.mutate() as mutar_contexto:
        for chave, valor in relacao_chave_valor.items():
            mutar_contexto[chave] = valor

        novo_contexto = mutar_contexto.finish()

    return novo_contexto


def atualizar_contexto_runtime(contexto_runtime: CONTEXTO_RUNTIME, chave: CONTEXTO_RUNTIME_KEYS, atualizar_func: CONTEXTO_RUNTIME_FUNC_ATUALIZAR) -> CONTEXTO_RUNTIME:
...
def atualizar_varios_valores_contexto_runtime(contexto_runtime: CONTEXTO_RUNTIME, relacao_chave_atualizar_func: Mapping[CONTEXTO_RUNTIME_KEYS, CONTEXTO_RUNTIME_FUNC_ATUALIZAR]) -> CONTEXTO_RUNTIME:
    chave_valor_atualizado = dict(
        [
            [chave, atualizar_func(
                ler_contexto_runtime(contexto_runtime, chave))]
    ...
    contexto_atualizado = escrever_varios_valores_contexto_runtime(
        contexto_runtime, chave_valor_atualizado)  # type: ignore
...
def _validar_varios_inputs(relacao_chave_valor: Mapping[CONTEXTO_RUNTIME_KEYS, Any]) -> bool:
    for chave, valor in relacao_chave_valor.items():
        _validar_input(chave, valor)
