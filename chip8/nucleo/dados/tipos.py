...
REGISTRADOR_INDEX = NewType("REGISTRADOR_INDEX", Map[Literal["i"], Optional[str]])
...
PIXEL_MAP = NewType(
    "PIXEL_MAP",
    Map[Tuple[PIXEL_MAP_COORDENADA_X, PIXEL_MAP_COORDENADA_Y], Literal[0, 1]],
)

R = TypeVar("R")
FUNCOES_EXECUCAO = Callable[[R], R]

CONTEXTO_RUNTIME_KEYS = Literal[
    "ram",
    "registradores",
    "registrador_index",
    "contador",
...
def e_instrucao(obj: Any) -> TypeGuard[INSTRUCAO_COMPLETA_CHIP8]:
    return all(
        [
            isinstance(obj, str),
            len(obj) == 4,
            all([(digito in SEQUENCIA_ALGARISMOS_HEXADECIMAIS) for digito in obj]),
        ]
    )


def e_ram(obj: Any) -> TypeGuard[RAM]:
    valores = all([(val == None or type(val) == str) for val in obj.values()])
    keys = all([(type(val) == str) for val in obj.keys()])

    return all([keys, valores])


def e_registrador(obj: Any) -> TypeGuard[REGISTRADORES]:
    valores = all([(val == None or isinstance(val, str)) for val in obj.values()])

    keys = all([val in SEQUENCIA_ALGARISMOS_HEXADECIMAIS for val in obj.keys()])
...
def e_pixel_map(obj: Any) -> TypeGuard[PIXEL_MAP]:
    valores = all([isinstance(val, int) for val in obj.values()])

    keys = all([isinstance(val, tuple) for val in obj.keys()])
